...
def kaishi():
    cmd = r'C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe --remote-debugging-port=9876'
    subprocess.run(cmd)
    time.sleep(5)
...
def mainpart():
    ...
    driver.get("https://hotels.ctrip.com/hotels/list?countryId=1&city=207&provinceId=0&checkin=2024/08/08&checkout=2024/08/09&optionId=206&optionType=City&directSearch=0&display=%E9%95%BF%E6%B2%99&crn=1&adult=1&children=0&searchBoxArg=t&travelPurpose=0&ctm_ref=ix_sb_dl&domestic=1&")
...
thread1 = threading.Thread(target = kaishi , args = ())
thread2 = threading.Thread(target = mainpart , args = ())
